...
//-----------------------------------------
	...
	ds1820_init(DS1820_pin_pm);              //Initialize DS1820 Buffer mitte
	ds1820_init(DS1820_pin_pu);              //Initialize DS1820 Buffer unten
...
//-----------------------------------------
		...
		temp_pu = ds1820_read_temp(DS1820_pin_pu);//Get temperature from DS1820 puffer unten
		temp_ke	= ds1820_read_temp(DS1820_pin_ke);//Get temperature from DS1820 kessel
		temp_bo	= ds1820_read_temp(DS1820_pin_bo);//Get temperature from DS1820 boiler
		...
		sprintf(temp_pu_str,"%.1f C",temp_pu); //Convert temp. puffer unten to string
		sprintf(temp_ke_str,"%.1f C",temp_ke); //Convert temp. kessel to string
		sprintf(temp_bo_str,"%.1f C",temp_bo); //Convert temp. boiler to string
	}
