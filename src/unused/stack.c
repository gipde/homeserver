...
########################################################*/
int checksum (unsigned char* pointer, unsigned long result32,
...
{
    if (buffer[0] == mymac[0] && buffer[1] == mymac[1] &&
            buffer[2] == mymac[2] && buffer[3] == mymac[3] &&
            buffer[4] == mymac[4] && buffer[5] == mymac[5])return (1);
...
{
    if (buffer[38] == myip[0] && buffer[39] == myip[1] &&
            buffer[40] == myip[2] && buffer[41] == myip[3])return (1);
...
void icmp(unsigned int len, unsigned char* buff)
{
    ...
    if (icmp->ICMP_checksumByteOne >  0xFF - 0x08)
        icmp->ICMP_checksumByteTwo++;
...
void udp(unsigned int len, unsigned char* buff)
{
    ...
    buff[17] = ((IP_UDP_HEADERLENGHT + len) &
                0x00FF); // ip header lenght anpassen
